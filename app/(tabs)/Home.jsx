...
const Home = () => {
  ...
  const [fetchError, setFetchError] = useState(false); // Track if there was an error in fetching data

  useEffect(() => {
    // Start fetching city data
    fetch("http://ashtavinayak.somee.com/api/City/")
      ...
      .catch((error) => {
        ...
        setLoading(false);
        setFetchError(true); // Set fetch error flag if something goes wrong
      });
  }, []);

  const handlePress = (cityId, cityName) => {
    // Only navigate if data is successfully fetched and there is no fetch error
    if (!loading && !fetchError) {
      ...
      // Show a message or alert if data fetching is still in progress or failed
      console.log("Data is still loading or fetch failed.");
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.citySelection}>
        ...
        {/* If fetch failed, show an error message */}
        {fetchError && (
          <Text style={styles.errorMessage}>Failed to fetch city data. Please try again later.</Text>
        )}
      </View>
